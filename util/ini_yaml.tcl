...
# yaml2ini's two-branch tcllib gate: under any `package require aurig::core`
# load the schema module is present (core.tcl sources schema/manifest.tcl at
# :71), so require_libs fires and raises with {AURIG SCHEMA MANIFEST}. The
# plain-error branch below only fires when util/ini_yaml.tcl is sourced
# standalone (schema module NOT loaded), matching yaml2ini's branch B
# contract. require_libs always raises on a missing package, so the plain
# error is unreachable in the schema-loaded case -- there is no fall-through
# path that could return the empty string.
...
proc ::aurig::core::util::yaml2ini {inYaml outIni} {
    ...
    if {[llength [info commands ::aurig::core::schema::require_libs]]} {
        ...
        error "yaml2ini requires the tcllib 'yaml' package; there is no safe\
 lite-parser fallback for canonical YAML (for example, a block scalar -- a\
 `|` value with continuation lines -- is silently truncated to the literal\
 `|` character). Install tcllib (e.g. 'apt-get install tcllib') and ensure\
 it is on the Tcl auto_path. For other platforms, see the Requirements\
